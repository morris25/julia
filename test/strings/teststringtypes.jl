...
Base.next(sp::T, i::Int64) where {T<:Base.StringLIPairs{<:DecodeString}} = invoke(Base.next, Tuple{T, Any}, sp, i)
Base.start(sp::Base.StringLIPairs{<:DecodeString}, state) = start(sp.s)
Base.done(sp::Base.StringLIPairs{<:DecodeString}, state) = done(sp.s, state)
